...
class UserController extends AdminbaseController
{
    ...
    function index()
    {
        $count = $this->users_model->where(["user_type" => 1])->count();
                ...
                if ($this->users_model->create()) {
                    ...
                    if ($result !== FALSE) {
                        ...
                        foreach ($role_ids as $role_id) {
                            $role_user_model->add(["role_id" => $role_id, "user_id" => $result]);
                        }
    ...
    function edit()
    {
        ...
        $this->assign("roles", $roles);
        $role_user_model = M("RoleUser");
        $role_ids        = $role_user_model->where(["user_id" => $id])->getField("role_id", TRUE);
            ...
            if (!empty($_POST['role_id']) && is_array($_POST['role_id'])) {
                ...
                if ($this->users_model->create()) {
                    ...
                    if ($result !== FALSE) {
                        $uid             = intval($_POST['id']);
                        $role_user_model = M("RoleUser");
                        $role_user_model->where(["user_id" => $uid])->delete();
                        foreach ($role_ids as $role_id) {
                            $role_user_model->add(["role_id" => $role_id, "user_id" => $uid]);
                        }
                        $this->success("保存成功！");
                    } else {
                        $this->error("保存失败！");
    ...
    function ban()
    {
        $id = intval($_GET['id']);
        if ($id) {
            $rst = $this->users_model->where(["id" => $id, "user_type" => 1])->setField('user_status', '0');
            if ($rst) {
    ...
    function cancelban()
    {
        $id = intval($_GET['id']);
        if ($id) {
            $rst = $this->users_model->where(["id" => $id, "user_type" => 1])->setField('user_status', '1');
            if ($rst) {
